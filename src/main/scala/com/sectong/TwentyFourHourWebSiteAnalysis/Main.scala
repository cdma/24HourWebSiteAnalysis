...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    //    val Array(zkQuorum, group, topics, numThreads) = Array("hadoop2", "groups", "test", "1") // 本地测试运行
    ...
    windowDStream.foreachRDD(line => {
      if (line.count() == 0) {
        ...
        val df = sqlContext.createDataFrame(line) //创建DataFrame
        df.registerTempTable("logs")
        val top10SrcIp24Hour = sqlContext.sql("SELECT ipAddress, COUNT(*) as count FROM logs GROUP BY ipAddress ORDER BY count DESC LIMIT 100")
